/// @description scr_menu_joint_friction();
function dks_setter_joint_friction() 
{
 // The script "dks_button_press" check if the mouse are hover a button structure...
	...
	var p3 = dks_button_press(menu_b_joint_friction_info);
	var p4 = dks_button_press(menu_b_joint_friction_int);

	// Creating the input boxes  
	if (! input_created)
	{
	 // Create a list of inputboxes for the joint friction settings.	
	 input_list = dks_input_list_create(menu_b_joint_friction_int,target.joint_list,1);
	...
	if (p1 = -1) and (p2 = -1) and (p3 = -1) and (p4 =-1)
	{
	 with(target)
	 {
	  if (! keyboard_check(vk_control))
	  {
	   ...
	   {
		 // Set the coordinates of our anchor point  
	    joint_list[| e_joint_friction.anchor_x] = other.Mxd-x;
	    joint_list[| e_joint_friction.anchor_y] = other.Myd-y;
	    if not ds_exists(joint_offset, ds_type_list)
	    {
	     joint_offset = ds_list_create();
	    }
		// Get the offset of our anchor point relative to our Deco object
