...
function dks_object_paste(argument0, argument1, argument2) 
{
  ...
  if (have_tween)
	 {
	  if is_array(creator.save_tweenS)
	  {
	   ...
	   dks_tween_update(id,other.x,other.y);
	  }        
   ...
   if (is_physics)
    ...
    //physics_joint_variables
	   joint = creator.save_joint;
	   joint_target = creator.save_joint_target;
	   joint_parent = creator.save_joint_parent;
    if (joint)
	   {
	    joint_list = ds_list_create();
	    switch(phyprops[| physics.joint_type])
	    {
	     case "Joint distance" : ds_list_copy(joint_list,creator.save_joint_distance);break;
	     case "Joint rope"     : ds_list_copy(joint_list,creator.save_joint_rope);break;
	     case "Joint revolute" : ds_list_copy(joint_list,creator.save_joint_revolute);break;
	     case "Joint prismatic": ds_list_copy(joint_list,creator.save_joint_prism);break;
	     case "Joint gear"     : ds_list_copy(joint_list,creator.save_joint_gear);break;
	     case "Joint friction" : ds_list_copy(joint_list,creator.save_joint_friction);break;
	     case "Joint pulley"   : ds_list_copy(joint_list,creator.save_joint_pulley);break;
	     case "Joint weld"     : ds_list_copy(joint_list,creator.save_joint_weld);break;
	     case "Joint wheel"    : ds_list_copy(joint_list,creator.save_joint_wheel);break;
	    }
	     joint_offset = ds_list_create();
	     ds_list_copy(joint_offset,creator.save_joint_offset);
	   }
	  }
   
			parent = creator.save_parent;      
	  if(parent)
	  {
	   var size = array_length(other.child);
	   for (var i =0; i<size; i++)
	   {
     with(other.child[i])
	    {
	     dks_object_copy(id);
	     other.child[i] = dks_object_paste(argument0+bind_xoffset,argument1+bind_yoffset,object_index); 
	    }
			  child[i].bind_target = id;
			  child[i].bind_xoffset = other.child[i].bind_xoffset;
			  child[i].bind_yoffset = other.child[i].bind_yoffset;
			  child[i].zoff = other.child[i].zoff;
			  with(child[i])
			  {
		    if (other.joint)
			   {
			    other.joint_target = id;
			   }
    
						if (joint)
			   {//experimental chain joint.......mirar!!
			    if (i =0)
			    {
			     joint_target = other.id;
			    }
			    else
			    {
			     joint_target = other.child[i-1].id;
			    }
		    }
		   }
	   }
   ...
   dks_object_copy(other.id);// Recopy the last parent copy to repaste this if you want
		 with(obj_deco_creator)
		 {
		  dks_undo_write(undo_map,other.id,1);
		 }
   if(object_execute)
		 {
		  if(perform_event>-1)
