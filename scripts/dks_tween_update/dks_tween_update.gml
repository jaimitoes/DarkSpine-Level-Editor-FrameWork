/// @description dks_tween_update( id, xoffset, yoffset)
/// @param  id
/// @param  xoffset
/// @param  yoffset
function dks_tween_update(argument0, argument1, argument2) 
{
	with(argument0)
	{
	 if (have_tween)
	 {
	  ...
	  var stx = argument1, sty = argument2;
	  for (var i =0; i<s; i++)
	  {
	   if (tween[i] !=-1)
	   {
	    ...
	    {
	     case 0 : 
	     TweenSet(tween[0],"start", st + (x -stx));
	     TweenSet(tween[0],"destination", dt + (x-stx));
	     break;
	     case 1 :
	     TweenSet(tween[1],"start", st + (y -sty));
	     TweenSet(tween[1],"destination", dt + (y -sty));
	     break;
	     case 2 :
