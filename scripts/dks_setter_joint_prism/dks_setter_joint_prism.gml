...
function dks_setter_joint_prism() 
{
 // The script "dks_button_press" check if the mouse are hover a button structure...
	...
	var p3 = dks_button_press(menu_b_joint_prism_info);
	var p4 = dks_button_press(menu_b_joint_prism_int);

	// Creating the input boxes  
	if (! input_created)
	{
	 // Create a list of inputboxes for the joint prismatic settings.
	 input_list = dks_input_list_create(menu_b_joint_prism_int,target.joint_list, 1);
	...
	if (p1 = -1) and (p2 = -1) and (p3 = -1) and (p4 =-1)
	{
	 with(target)
	 {
	  ...
	  {  
	   ...
	   {
		// Set the coordinates of our first point
	    joint_list[| e_joint_prism.w_anchor_x] = global.mouse_xd-x;
	    joint_list[| e_joint_prism.w_anchor_y] = global.mouse_yd-y;
		// Get the offset of our first point relative to our Deco object
	   ...
	   {
		// Set the coordinates of our second point
	    joint_list[| e_joint_prism.w_axis_x] = global.mouse_xd-x;
	    joint_list[| e_joint_prism.w_axis_y] = global.mouse_yd-y;
		// Get the offset of our second point relative to our Deco object
